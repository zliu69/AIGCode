...
from hf_olmo.configuration_olmoxmoe import OLMoXMoEConfig
# from .configuration_olmoxmoe import OLMoXMoEConfig
from hf_olmo.modeling_olmoxmoe import OLMoXMoEForCausalLM
# from .modeling_olmoxmoe import OLMoXMoEForCausalLM

# from hf_olmoxmoe.tokenization_olmo_fast import OLMoTokenizerFast
...
def write_config(checkpoint_dir: str, to_hf_dir: str):
    ...
    config = OLMoXMoEConfig(**config_kwargs)

    logger.info(f"Saving HF-compatible config to {os.path.join(checkpoint_dir, 'config.json')}")
    config.save_pretrained(to_hf_dir)


def write_model(checkpoint_dir: str, to_hf_dir: str, ignore_olmo_compatibility: bool = False):
    ...
    new_state_dict = {f"{OLMoXMoEForCausalLM.base_model_prefix}.{key}": val for key, val in state_dict.items()}
...
def write_tokenizer(checkpoint_dir: str):
    ...
    tokenizer = OLMoXMoETokenizerFast(
...
def convert_checkpoint(checkpoint_dir: str, to_hf_dir: str, ignore_olmo_compatibility: bool = False):
    if not os.path.exists(to_hf_dir):
        os.makedirs(to_hf_dir)
    write_config(checkpoint_dir, to_hf_dir)
    write_model(checkpoint_dir, to_hf_dir, ignore_olmo_compatibility=ignore_olmo_compatibility)
...
def main():
    ...
    parser.add_argument(
        "--to-hf-dir",
        default="/sharedata/zimoliu/models/olmoxmoe_252000",
    ...
    convert_checkpoint(args.checkpoint_dir, args.to_hf_dir, args.ignore_olmo_compatibility)

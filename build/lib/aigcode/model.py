...
class RotaryEmbedding(nn.Module):
    ...
    def get_rotary_embedding(self, seq_len: int, device: torch.device) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(device.type, enabled=False):
            ...
            inv_freq = 1.0 / (self.config.rope_base ** (torch.arange(0, dim, 2, device=device, dtype=torch.float) / dim))
...
class OLMoBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if config.use_xmoe and layer_id % config.moe_freq == 0:
            # self.ff_out = nn.Linear(
            #     int(self.act.output_multiplier * self.hidden_size / config.exp_dim_ratio),
            #     config.d_model,
    ...
    def reset_parameters(self):
        ...
        if self.config.init_fn == InitFnType.normal:
            # std = math.sqrt(2.0 / (self.ff_proj.in_features + self.ff_proj.out_features))
            #     ff_out_std = math.sqrt(2.0 / (self.ff_out.in_features + self.ff_out.out_features)) / (math.sqrt(2 * (self.layer_id + 1)))

            # attn_out_std = 1.0 / (math.sqrt(2 * self.attn_out.in_features * (self.layer_id + 1)))
            attn_out_std = self.config.init_std / (math.sqrt(2 * (self.layer_id + 1)))
            # attn_out_std = (math.sqrt(1.0 / 3.0 / self.attn_out.in_features)) / (math.sqrt(2 * (self.layer_id + 1)))
            # attn_out_std = math.sqrt(2.0 / (self.attn_out.in_features + self.attn_out.out_features)) / (math.sqrt(2 * (self.layer_id + 1)))
            if self.ff_out:
                # ff_out_std = 1.0 / (math.sqrt(2 * self.ff_out.in_features * (self.layer_id + 1)))
                # ff_out_std = (math.sqrt(1.0 / 3.0 / self.ff_out.in_features)) / (math.sqrt(2 * (self.layer_id + 1)))
                ff_out_std = self.config.init_std / (math.sqrt(2 * (self.layer_id + 1)))
                # ff_out_std = math.sqrt(2.0 / (self.ff_out.in_features + self.ff_out.out_features)) / (math.sqrt(2 * (self.layer_id + 1)))
...
class OLMoExpert(nn.Module):
    ...
    def __init__(self, config, layer_id):
        ...
        # Share MLP layer
        self.act = Activation.build(config)
        self.ff_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=False)
        self.ff_out = nn.Linear(int(self.act.output_multiplier * self.intermediate_size), self.hidden_size, bias=False)
    ...
    def forward(self, x):
        ...
        # else:
        return self.ff_out(self.act(self.ff_proj(x)))
    ...
    def reset_parameters(self):
        if not self.reset_params:
            if self.config.init_fn == InitFnType.normal:
                ...
                # print("block_layer_id :{}, moe output_proj_std :{}\n".format(self.layer_id, ff_out_std))
                cutoff_factor = self.config.init_cutoff_factor
            elif self.config.init_fn == InitFnType.mitchell:
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        if self.config.use_xmoe and layer_id % config.moe_freq == 0:
            self.use_moe = self.config.use_xmoe
            if self.config.moe_top1_expert:
                self.gate = Top1Gate(
                    self.config,
                    self.config.d_model,
                    self.config.moe_expert_count,
                    self.layer_id,
                    use_fp32=self.config.moe_gating_use_fp32,
                    moe_eval_capacity_token_fraction=getattr(self.config, "moe_eval_capacity_token_fraction", 0.25),
                )
            else:
                self.gate = Top2Gate(
                    self.config,
                    ...
                    self.config.moe_expert_count,
                    self.layer_id,
                    self.config.moe_gating_use_fp32,
                    self.config.moe_second_expert_policy,
                    self.config.moe_normalize_gate_prob_before_dropping,
                    getattr(self.config, "moe_eval_capacity_token_fraction", 0.25),
                    getattr(self.config, "moe_batch_prioritized_routing", False),
                )
            self.experts = self.make_experts()
        else:
            self.use_moe = False
    ...
    def make_experts(self):
        expert_list = []
        for _ in range(self.config.moe_expert_count + self.config.moe_share_expert_count):
            ...
            # ))
            expert_list.append(OLMoExpert(self.config, self.layer_id))
    ...
    def reset_parameters(self):
        ...
        if self.config.init_fn == InitFnType.normal:
            std = self.config.init_std
            # att_proj_std = math.sqrt(2.0 / (self.att_proj.in_features + self.att_proj.out_features))
            # if not self.use_moe:
            #     ff_proj_std = math.sqrt(2.0 / (self.ff_proj.in_features + self.ff_proj.out_features))
            cutoff_factor = self.config.init_cutoff_factor
        elif self.config.init_fn == InitFnType.mitchell:
            if (not self.use_moe 
            and self.config.moe_freq !=0 
            and self.layer_id % self.config.moe_freq == 1):
        ...
        # init_normal(self.att_proj, att_proj_std, cutoff_factor)
        ...
        if self.use_moe:
            ...
            init_normal(self.ff_proj, std, cutoff_factor)
            # init_normal(self.ff_proj, ff_proj_std, cutoff_factor)
    ...
    def forward(
        ...
        layer_past: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
        use_cache: bool = False,
    ) -> Tuple[torch.Tensor, Optional[Tuple[torch.Tensor, torch.Tensor]], Optional[torch.Tensor], Optional[Dict]]:
        if self.config.layer_std_check:
            print("block_layer_id :{}, input_std :{}\n".format(self.layer_id, x.std()))
        ...
        if self.use_moe:
            # x_shape = x
            reshaped_input = x.reshape(-1, self.config.d_model)
            l_aux, combine_weights, _, metadata = self.gate(reshaped_input, None)
            # S, M = reshaped_input.size(0), reshaped_input.size(1)
            if self.config.moe_share_expert_count > 0:
                combine_weights = F.pad(combine_weights, (0, 0, 0, self.config.moe_share_expert_count), "constant", 1.0)
            ...
            for model, chunk in zip(self.experts, chunks) :
                ...
                # results.append(model(reshaped_input) * chunk.permute(1, 0) * float(self.config.moe_expert_count / 2)) 
            ...
            # x = experts_output.reshape(x.size(0), x.size(1), int(self.hidden_size / self.config.exp_dim_ratio))
            x = experts_output.reshape(x.size(0), x.size(1), x.size(2))
            # x = self.
            if self.config.layer_std_check:
                print("block_layer_id :{}, moe output_std :{}\n".format(self.layer_id, x.std()))
        else:
            x = self.ff_proj(x)
            ...
            if self._activation_checkpoint_fn is not None:
                ...
                x = self.ff_out(x)
            if self.config.layer_std_check:
                print("block_layer_id :{}, output_std :{}\n".format(self.layer_id, x.std()))
            l_aux = None
        x = self.dropout(x)
        x = og_x + x
        if self.config.layer_std_check:
            print("block_layer_id :{}, residual_output_std :{}\n".format(self.layer_id, x.std()))
...
class OLMoOutput(NamedTuple):
    ...
    l_aux: Optional[torch.Tensor]
    """
    Auxiliary loss for moe layer.
    """

    meta_data: Optional[Dict]
    """
    Meta data during model forward.
    """
...
class OLMo(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.config.init_fn == InitFnType.normal:
            ...
            # wte_std = math.sqrt(2.0 / (self.transformer.wte.num_embeddings + self.transformer.wte.embedding_dim))
            print("wte, _std :{}\n".format(wte_std))
        ...
        if hasattr(self.transformer, "ff_out"):
            if self.config.init_fn == InitFnType.normal:
                ...
                # ff_out_std = math.sqrt(2.0 / (self.transformer.ff_out.in_features + self.transformer.ff_out.out_features))
                print("transformer ff out, _std :{}\n".format(ff_out_std))
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if should_checkpoint_block(self.activation_checkpointing_strategy, block_idx):
                    # shape: (batch_size, seq_len, d_model)
                    x, cache, l_aux, meta_data = self._activation_checkpoint_fn(
                        block, x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache
                    )
                else:
                    # shape: (batch_size, seq_len, d_model)
                    x, cache, l_aux, meta_data = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache)

                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
                all_l_aux.append(l_aux)
        ...
        if self.config.use_xmoe and self.config.moe_auxiliary_loss:
            # MoE loss params.
            gate_loss = 0.0
            gate_count = 0
            # Compute MoE auxiliary loss.
            for l_aux in all_l_aux:
                if l_aux is not None:
                    gate_loss = gate_loss + l_aux
                    gate_count = gate_count + 1
            if self.config.moe_gate_loss_combine_method == "average" and gate_count > 0:
                gate_loss = gate_loss / gate_count
            # print("l_aux in olmo: {}\n".format(gate_loss))
        else:
            gate_loss = None

        if self.config.use_xmoe and self.config.moe_logging:
            final_meta_data = {}
            final_meta_data_kc = {}
            ...
            # print("moe meta data in olmo: {}\n".format(final_meta_data))
        else:
        ...
        return OLMoOutput(logits=logits, attn_key_values=attn_key_values, hidden_states=tuple(all_hidden_states) if output_hidden_states else None, l_aux=gate_loss, meta_data=final_meta_data)  # type: ignore[arg-type]
    ...
    def get_fsdp_wrap_policy(self, wrap_strategy: Optional[FSDPWrapStrategy] = None):
        ...
        if wrap_strategy == FSDPWrapStrategy.by_block:
            ...
            def fsdp_wrap_fn(module, recurse: bool = True, nonwrapped_numel: int = 0):
                ...
                or isinstance(module, OLMoExpert) 
                # or isinstance(module, MOELayer) 
                # or isinstance(module, MOEV2Layer)
                # or isinstance(module, MOEV3Layer)
                # or isinstance(module, AigcodeSimpleBlock) 
                # or isinstance(module, Top1Gate) 
                # or isinstance(module, Top2Gate) 
                # or isinstance(module, FeedForwardNetwork) 

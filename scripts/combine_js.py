...
def main(
    ...
    # print(recursively_list_files(src))
    exploded_src = list(set(path for path in os.listdir(src) if (path.startswith("gsm8k") and path.endswith(".json") and "sample_lst" in path)))
    print("exploded_src: {}\n".format(exploded_src))
    ...
    for parq_f in exploded_src:
        output_file = os.path.join(output, "{}.json".format(parq_f.rsplit(".",1)[0]))
        print("output_file: {}\n".format(output_file))
        print("parquet_file: {}\n".format(parq_f))
        ...
        rec_lst = []
        with open(
            os.path.join(src, parq_f),
            "r", encoding="utf-8") as f:
            for line in f.readlines():
                json_obj = json.loads(line.strip())

                js_lst.append(json_obj)
        try:
            # if "question" in ds_train.column_names and "answer" in ds_train.column_names:
            for js in js_lst:
                if "\n####" not in js["题目改写"]["answer"]:
                    js["题目改写"]["answer"] = js["题目改写"]["answer"] + "\n#### " + js["answer"].split("\n#### ")[1]
                rec_lst.append(js["题目改写"])
            
                
            with open(output_file, "a+", encoding='utf-8') as f:
                f.write("\n".join([json.dumps(j, ensure_ascii=False) for j in rec_lst]))
            print("js len: {}\n".format(len(rec_lst)))

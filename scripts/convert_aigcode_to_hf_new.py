...
Sample usage:
```
python src/transformers/models/aigcode/convert_aigcode_weights_to_hf.py \
    --input_dir /path/to/downloaded/aigcode/weights --model_size 7B --output_dir /output/path
```
...
def write_model(
    ...
    aigcode_config = yaml.safe_load(config_path.read_text())["model"]

    n_layers = aigcode_config["n_layers"]
    n_heads = aigcode_config["n_heads"]
    dim = aigcode_config["d_model"]
    dims_per_head = dim // n_heads
    base = 10000.0
    inv_freq = 1.0 / (base ** (torch.arange(0, dims_per_head, 2).float() / dims_per_head))
    max_position_embeddings = aigcode_config["max_sequence_length"]

    vocab_size = aigcode_config.get("embedding_size", aigcode_config["vocab_size"])

    if aigcode_config.get("n_kv_heads", None) is not None:
        num_key_value_heads = aigcode_config["n_kv_heads"]  # for GQA / MQA
    elif aigcode_config["multi_query_attention"]:  # compatibility with other checkpoints
    ...
    if aigcode_config.get("mlp_hidden_size", None) is not None:
        intermediate_size = aigcode_config["mlp_hidden_size"] // 2
    else:
        intermediate_size = (dim * aigcode_config["mlp_ratio"]) // 2

    if fix_eos_token_id and aigcode_config["eos_token_id"] == 0:
        # Fixing a bug in AIGCcode where eos token id was incorrectly set
        print("Changing eos_token_id from 0 to 50279.")
        aigcode_config["eos_token_id"] = 50279
    ...
    config = OlmoConfig(
        ...
        pad_token_id=aigcode_config["pad_token_id"],
        bos_token_id=None,
        eos_token_id=aigcode_config["eos_token_id"],
        tie_word_embeddings=aigcode_config.get("weight_tying", True),
        rope_theta=base,
        clip_qkv=aigcode_config.get("clip_qkv"),
    )
    ...
    print("Loading the checkpoint in a AIGCcode model.")
...
def main():
    ...
    parser.add_argument(
        "--input_dir",
        required=True,
        help="Location of AIGCcode weights, which contains config.yaml and model.pt.",
    )
    parser.add_argument(
        "--no_tokenizer",
        action="store_false",
        dest="include_tokenizer",
        help="If set, do not convert AIGCcode tokenizer to HF tokenizer.",
    )
    parser.add_argument(
        "--tokenizer_json_path",
        type=Path,
        default=None,
        help="Location of AIGCcode tokenizer json file. Defaults to what is set in the config file.",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
    ...
    # Different AIGCcode versions used different default values for max_position_embeddings, hence the need to be able to specify which version is being used.

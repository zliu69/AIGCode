...
from aigcode import AIGCcode
from aigcode.config import TrainConfig
from aigcode.data import build_train_dataloader
from aigcode.model import LayerNormBase
from aigcode.optim import build_optimizer, build_scheduler
from aigcode.torch_util import get_world_size, seed_all
...
def _apply_scheduler(cfg, step_count, scheduler, optimizer):
    """
    Apply scheduler according to AIGCcode style.
    """
...
def _naive_train_loop(
    ...
    for epoch in range(max_epochs):
        for idx, batch in enumerate(data_loader):
            ...
            loss_b.backward()
            aigcode_grad_norm = optimizer_b.clip_grads_and_collect_metrics(step_count, device=torch.device(device))[
                "total_grad_norm"
            ]
            ...
            assert (
                torch.abs(torch_grad_norm - aigcode_grad_norm) < 1e-6
            ), "grad norms computed by torch and AIGCcode codebase are different"

            if step_count == max_iterations:
                break


def _run_aigcode_optim_againt_torch_optim(
    max_iterations: int,
    max_norm: float,
    device: str,
):
    ...
    model_a = AIGCcode(cfg.model).to(device)
    ...
    aigcode_optimizer = build_optimizer(cfg, model_b)
...
def test_aigcode_optimizer_and_clipping_cpu(max_iterations, max_norm, device):
    _run_aigcode_optim_againt_torch_optim(max_iterations, max_norm, device)
...
def test_aigcode_optimizer_and_clipping_gpu(max_iterations, max_norm, device):
    _run_aigcode_optim_againt_torch_optim(max_iterations, max_norm, device)

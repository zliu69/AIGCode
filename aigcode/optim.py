...
def get_param_groups(cfg: TrainConfig, model: nn.Module) -> List[Dict[str, Any]]:
    ...
    """
    if cfg.moe_params_isolation:
        param_groups: List[Dict[str, Any]]
        param_group_defaults = {
            "sharded": isinstance(model, FullyShardedDataParallel),
            "max_grad_norm": cfg.max_grad_norm,
            "max_grad_norm_ratio": None if cfg.max_grad_norm else cfg.max_grad_norm_ratio,
        ...
        for mn, m in model.named_modules():
            for pn, p in m.named_parameters():
                ...
                # add exp param to exp group
                if "expert" in fpn:
                    expert_params.add(fpn)
                    all_params[fpn] = p
                    continue
                if "gate" in fpn and "wg" in fpn:
                    gate_params.add(fpn)
                    all_params[fpn] = p
                    continue
        ...
        union_params = (decay | no_decay) | expert_params | gate_params
        ...
        expert_sorted = sorted(list(expert_params))
        ...
        if len(expert_sorted) > 0:
            param_groups.append(
                {
                    "params": [all_params[pn] for pn in expert_sorted],
                    "param_names": expert_sorted,
                    "lr": cfg.optimizer.learning_rate * cfg.moe_expert_lr_scale,
                    "initial_lr": cfg.optimizer.learning_rate * cfg.moe_expert_lr_scale,
                    "sharded": isinstance(model, FullyShardedDataParallel),
                    "max_grad_norm": None,
                    "max_grad_norm_ratio": cfg.max_grad_norm_ratio * cfg.moe_grad_norm_scale if cfg.max_grad_norm_ratio else cfg.max_grad_norm * cfg.moe_grad_norm_scale,
                    "initial_max_grad_norm": None,
                    "initial_max_grad_norm_ratio": cfg.max_grad_norm_ratio * cfg.moe_grad_norm_scale if cfg.max_grad_norm_ratio else cfg.max_grad_norm * cfg.moe_grad_norm_scale,
                
                }
            )

        if len(gate_sorted) > 0:
            param_groups.append(
                {
                    "params": [all_params[pn] for pn in gate_sorted],
                    "param_names": gate_sorted,
                    "weight_decay": cfg.moe_gate_weight_decay,
                    "lr": cfg.optimizer.learning_rate * cfg.moe_gate_lr_scale,
                    "initial_lr": cfg.optimizer.learning_rate * cfg.moe_gate_lr_scale,
                    "sharded": isinstance(model, FullyShardedDataParallel),
                    "max_grad_norm": None,
                    "max_grad_norm_ratio": cfg.max_grad_norm_ratio * cfg.moe_gate_grad_norm_scale if cfg.max_grad_norm_ratio else None,
                    "initial_max_grad_norm": None,
                    "initial_max_grad_norm_ratio": cfg.max_grad_norm_ratio * cfg.moe_gate_grad_norm_scale if cfg.max_grad_norm_ratio else None,

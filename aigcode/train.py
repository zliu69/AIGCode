...
class Trainer:
    ...
    def model_forward(
        ...
        logits, _, _, l_aux, _ = self.dist_model(
        ...
        return ce_loss, z_loss, logits, l_aux

    def train_micro_batch(
        self, micro_batch: Dict[str, Any], batch_size_in_tokens: int
    ) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
        ce_loss, z_loss, logits, l_aux = self.model_forward(
    ...
    def train_batch(self, batch: Dict[str, Any]) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        for micro_batch_idx, micro_batch in enumerate(micro_batches):
            ...
            with grad_sync_context():
                with torch.autocast("cuda", enabled=True, dtype=self.cfg.autocast_precision):
                    # Run forward pass.
                    loss, ce_loss, z_loss, l_aux = self.train_micro_batch(micro_batch, batch_size_in_tokens)
        ...
        return ce_batch_loss, z_batch_loss, l_aux_batch_loss
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        ce_batch_loss, z_batch_loss, l_aux_batch_loss = self.train_batch(batch)
